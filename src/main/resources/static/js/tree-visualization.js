...
class FamilyTreeVisualization {
    constructor(containerId, treeId) {
        ...
        this.emptyState.innerHTML = '<div style="display:flex; justify-content:center; align-items:center; height:100%; color:#fff;"><div style="text-align:center;"><div style="font-size:40px;margin-bottom:10px;">ðŸ‘¤</div><p>ChÆ°a cÃ³ thÃ nh viÃªn nÃ o. Nháº¥n "ThÃªm thÃ nh viÃªn" Ä‘á»ƒ báº¯t Ä‘áº§u.</p><button class="btn btn-primary" onclick="if(window.personFormManager) window.personFormManager.openAddRootForm()">+ ThÃªm thÃ nh viÃªn Ä‘áº§u tiÃªn</button></div></div>';
    ...
    setupFilters() {
        ...
        filterGeneration.innerHTML = '<option value="">-- Chá»n Ä‘á»i --</option>';
        sortedGens.forEach(gen => {
            const option = document.createElement('option');
            option.value = gen;
            option.textContent = `Äá»i thá»© ${gen}`;
        ...
        if (!this.filterEventsBound) {
            filterGeneration.addEventListener('change', () => {
                ...
                filterBranch.innerHTML = '<option value="">-- Chá»n nhÃ¡nh --</option>';
                if (gen) {
    ...
    async loadData() {
        try {
            ...
            showToast("Lá»—i khi táº£i dá»¯ liá»‡u gia pháº£ (Failed to load tree)", "error");
        }
    }
    ...
    render(treeData) {
        ...
        const node = this.g.selectAll(".node")
            ...
            .on("click", (event, d) => {
                if(!d.data.id) {
                    showToast("ÄÃ¢y lÃ  Thá»§y Tá»• áº£o (gom nhÃ³m cÃ¡c nhÃ¡nh rá»i ráº¡c). KhÃ´ng thá»ƒ xem chi tiáº¿t.", "info");
                    return;
                }
        ...
        btnMenu.append("text").text("â‹®").attr("text-anchor", "middle").attr("y", 5).attr("fill", "white").attr("font-weight", "bold").attr("font-size", "18px");
        ...
        nodesWithSpouse.forEach(d => {
            d.data.spouses.forEach((spouse, index) => {
                ...
                this.drawSvgNodes(spouseGroup, spouse, d.depth + 1);
                ...
                spouseBtnMenu.append("text").text("â‹®").attr("text-anchor", "middle").attr("y", 5).attr("fill", "white").attr("font-weight", "bold").attr("font-size", "18px");
            });
        });
    ...
    drawSvgNodes(selection, singleData = null, singleDepth = null) {
        ...
        selection.append('rect')
            ...
            .attr('rx', 10)
            .attr('fill', '#ffffff')
            .attr('stroke', d => { const dt = singleData || d.data; return dt.gender === 'NAM' ? '#1a56c4' : '#b5245e'; })
            .attr('stroke-width', 2)
        ...
        selection.append('circle')
            .attr('cx', 35)
            .attr('cy', this.nodeHeight / 2 + 5)
            .attr('r', 22)
            .attr('fill', d => { const dt = singleData || d.data; return dt.gender === 'NAM' ? '#dbeafe' : '#fce7f3'; })
            .attr('stroke', d => { const dt = singleData || d.data; return dt.gender === 'NAM' ? '#1a56c4' : '#b5245e'; })
            .attr('stroke-width', 2);
        ...
        selection.append('text')
            .attr('x', 70)
            .attr('y', this.nodeHeight / 2 - 2)
            .attr('font-weight', 'bold')
            .attr('font-size', '14px')
            .attr('fill', '#1e293b')
            .text(d => {
                const dt = singleData || d.data;
                const fullName = dt.name || [dt.ho, dt.tenDem, dt.ten].filter(Boolean).join(' ');
        ...
        selection.append('rect')
            ...
            .attr('height', 16)
            .attr('rx', 4)
            .attr('fill', d => { const dt = singleData || d.data; return dt.gender === 'NAM' ? '#1a56c4' : '#b5245e'; });
